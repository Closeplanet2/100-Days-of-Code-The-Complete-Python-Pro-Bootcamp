...
class TkinterController:
    ...
    def add_image(self, image_path, w=0, h=0, bg="white", pos_x=0, pos_y=0, g_pos_x=0, g_pos_y=0, destroy_status=None):
        if not self.current_window is None:
            return self.current_window.add_image(image_path, w, h, bg, pos_x, pos_y, g_pos_x, g_pos_y, destroy_status)
        return None
    ...
    def add_image_as_grid(self, card_image, w=5, h=5, pos_x=0, pos_y=0, g_pos_x=0, g_pos_y=0, offset_x=88, offest_y=129, numx=3, numy=2, index=0, destroy_status=None):
        if not self.current_window is None:
            return self.current_window.add_image_as_grid(card_image, w, h, pos_x, pos_y, g_pos_x, g_pos_y, offset_x, offest_y,
...
class TkinterClass(Tk):
    ...
    # Text, Background Color, Foreground Color, Width, Height, x_pos, y_pos, Font Size, Font Family, allow destroy
    def add_label(self, text="Text Here", bg="#000000", fg="#ffffff", w=0, h=0, x_pos=0, y_pos=0, g_pos_x=0, g_pos_y=0, fs=14, ff="Helvetica", destroy_status=None):
        label = Label(self, text=text, font=(ff, fs, "bold"), fg=fg, bg=bg)
    ...
    def add_image(self, image_path, w=0, h=0, bg="white", pos_x=0, pos_y=0, g_pos_x=0, g_pos_y=0, destroy_status=None):
        image = Image.open(image_path).resize((w, h)).convert("RGBA")
    ...
    def add_image_as_grid(self, image, w=0, h=0, pos_x=0, pos_y=0, g_pos_x=0, g_pos_y=0, offset_x=88, offest_y=129,
        ...
        card_image = image.resize((w, h)).convert("RGBA")
